...
The only bundled engine is deterministic and accepts inline positive prices with
the `buy_and_hold` or `flat` strategy. It is an adapter test engine, not a
production trading or alpha-discovery system. `agents/` and `workers/` are
module boundaries only; no Astra/Sol runtime or task queue is included.
