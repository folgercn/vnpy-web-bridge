...
- `validate_manifest(bundle_root, manifest, run)`：公共 Manifest Schema、Run 绑定、角色完整性、唯一条目/路径、内容定义、原始字节长度/摘要、载荷 Schema。返回已检查的载荷内容，不证明科学结论正确。
- `validate_handoff(request, objects, response=None)`：当前支持 `review_evidence` 的实际上下文、Run/Spec/Manifest/Evidence 引用、完整产物引用和实际 Review 与请求判据对应。其他操作明确拒绝，不能把结构合格等同跨对象验证完成。
