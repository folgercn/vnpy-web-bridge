...
def _wait_event(path: Path, event_type: str, timeout: float = 3.0) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        matches = [event for event in _events(path) if event["event_type"] == event_type]
        if matches:
            return matches[-1]
        time.sleep(0.01)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
        ...
        with connection:
            ...
            if request["operation"] == "query":
                response = {"state": "ACKNOWLEDGED", "broker_order_id": "loopback-order-001"}
            elif request["operation"] == "snapshot":
                generation = int(mode.split("-", 1)[1]) if mode.startswith("ack-") else 1
...
class _LoopbackGateway:
    ...
    def _call(self, operation: str, context: MutationContext | None = None) -> dict[str, Any]:
        request = {"operation": operation, "context": context.as_dict() if context else {}}
    ...
    def send_order(self, _request: Mapping[str, Any], context: MutationContext) -> dict[str, Any]:
        return self._call("send", context)

    def cancel_order(self, _request: Mapping[str, Any], context: MutationContext) -> dict[str, Any]:
        return self._call("cancel", context)

    def query_intent(self, _intent: SendIntent, context: MutationContext | None = None) -> dict[str, Any]:
...
def _start_gateway(context: multiprocessing.context.BaseContext, events: Path, mode: str) -> tuple[multiprocessing.Process, int]:
...
def _command(command: str, key: str, version: int, payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "actor": {"service": "control-api", "principal": "phase-c-process", "operator": "offline", "role": "admin"},
        "command": command,
...
def _ready_service(path: Path) -> tuple[ExecutionOrchestrator, object]:
    ...
    service = ExecutionOrchestrator(repo, NullGateway(), scope=SCOPE, environment=ENVIRONMENT, test_mode=True)
    token = service.acquire_leader("leader-process-mutation")
    service.process_command(_command("enable", "enable-process-000001", repo.state_version, {"authority_artifact_id": "artifact-process-001", "authority_hash": HASH, "expires_at": "2030-01-01T00:00:00Z", "reason": "offline process fault"}))
    snapshot_process, snapshot_port = _start_gateway(multiprocessing.get_context("spawn"), path.with_name("events.jsonl"), "ack")
    service.gateway = _LoopbackGateway(snapshot_port)
    service.process_command(_command("reconcile", "reconcile-process-001", repo.state_version, {"reconciliation_run_id": "run-process-001", "snapshot_id": "snapshot-phase-c-process-001", "reason": "offline process fault"}))
    snapshot_process.join(2)
    service.process_command(_command("start", "start-process-000001", repo.state_version, {"plan_id": "plan-process-000001", "plan_hash": HASH, "reason": "offline process fault"}))
    return service, token


def _send(service: ExecutionOrchestrator, token: object, key: str) -> dict[str, Any]:
    return service.send_order({"symbol": "RB", "volume": 1}, idempotency_key=key, plan_id="plan-process-000001", plan_hash=HASH, leader_epoch=token.epoch, fencing_token=token.fencing_token, token=token)
...
def run_process_faults(workdir: Path) -> list[dict[str, Any]]:
    ...
    leader_state, leader_events = workdir / "leader-state.json", workdir / "leader-events.jsonl"
    first = context.Process(target=_leader_worker, args=(str(leader_state), str(leader_events), "leader-process-a", True))
    first.start()
    ...
    time.sleep(0.15)
    second = context.Process(target=_leader_worker, args=(str(leader_state), str(leader_events), "leader-process-b", False))
    second.start(); second.join(2)
    time.sleep(0.15)
    third = context.Process(target=_leader_worker, args=(str(leader_state), str(leader_events), "leader-process-c", False))
    third.start(); third.join(2)
    os.kill(first.pid, signal.SIGKILL); first.join(2)
    _emit(str(leader_events), "leader_process_killed", {"pid": first.pid, "exitcode": first.exitcode})
    leader_snapshot = DurableExecutionRepository(leader_state, scope=SCOPE).snapshot()

    mutation_state, mutation_events = workdir / "mutation-state.json", workdir / "mutation-events.jsonl"
    ...
    restarted = ExecutionOrchestrator(DurableExecutionRepository(mutation_state, scope=SCOPE), _LoopbackGateway(query_port), scope=SCOPE, environment=ENVIRONMENT, test_mode=True)
    ...
    reconcile_result = restarted.process_command(_command("reconcile", "reconcile-process-002", restarted.repository.state_version, {"reconciliation_run_id": "run-process-002", "snapshot_id": "snapshot-phase-c-process-001", "reason": "same intent query closure"}))
    snapshot_process.join(2)

    send_crash_state, send_crash_events = workdir / "send-crash-state.json", workdir / "send-crash-events.jsonl"
    send_crash_service, send_crash_token = _ready_service(send_crash_state)
    send_kill_process, send_kill_port = _start_gateway(context, send_crash_events, "kill")
    ...
    after_send_kill = DurableExecutionRepository(send_crash_state, scope=SCOPE).snapshot()

    cancel_state, cancel_events = workdir / "cancel-state.json", workdir / "cancel-events.jsonl"
    ...
    with _expect(GatewayTimeout):
        cancel_service.cancel_order(accepted["intent_id"], idempotency_key="cancel-process-kill-001", plan_id="plan-process-000001", plan_hash=HASH, leader_epoch=cancel_token.epoch, fencing_token=cancel_token.fencing_token, token=cancel_token)
    kill_process.join(2)
    after_cancel = DurableExecutionRepository(cancel_state, scope=SCOPE).snapshot()

    return [
        {"case_id": "process_pause_lease_expiry_kill_restart", "records": _events(leader_events) + [{"event_type": "durable_leader_state", "payload": leader_snapshot}]},
        {"case_id": "loopback_partition_reset_unknown_restart_reconcile", "records": _events(mutation_events) + [{"event_type": "durable_state_after_reset", "payload": after_send}, {"event_type": "same_intent_query", "payload": query_result}, {"event_type": "reconcile_receipt", "payload": dict(reconcile_result.receipt)}]},
        {"case_id": "loopback_send_cancel_crash_boundaries", "records": _events(send_crash_events) + [{"event_type": "durable_state_after_send_kill", "payload": after_send_kill}] + _events(cancel_events) + [{"event_type": "durable_state_after_cancel_kill", "payload": after_cancel}]},
    ]
...
class _expect:
    ...
    def __exit__(self, exc_type: type[BaseException] | None, _value: object, _traceback: object) -> bool:
        if exc_type is None:
