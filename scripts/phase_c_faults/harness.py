...
def _ready_service() -> tuple[
    ...
    service.process_command(
        _command(
            "enable",
            "enable-phase-c-000001",
            repo.state_version,
            {
                "authority_artifact_id": "artifact-phase-c-000001",
                "authority_hash": HASH,
                "expires_at": "2030-01-01T00:00:00Z",
                "reason": "offline phase-c harness",
            },
        )
    )
    service.process_command(
        _command(
            "reconcile",
            "reconcile-phase-c-001",
            repo.state_version,
            {
                "reconciliation_run_id": "run-phase-c-000001",
                "snapshot_id": "snapshot-default",
                "reason": "offline fresh snapshot",
            },
        )
    )
    service.process_command(
        _command(
            "start",
            "start-phase-c-000001",
            repo.state_version,
            {
                "plan_id": "plan-phase-c-000001",
                "plan_hash": HASH,
                "reason": "offline phase-c plan",
            },
        )
    )
...
def _context(action: str, *, epoch: int = 3, token: int = 7) -> MutationContext:
    context = MutationContext(
        account_scope="account:phase-c",
        environment="simnow",
        leader_epoch=epoch,
        fencing_token=token,
        plan_id="plan-phase-c-000001",
        plan_hash=HASH,
        intent_id=f"intent-phase-c-{action}-001",
        idempotency_key=f"{action}-phase-c-key-0001",
        action=action,
        receipt_id=f"receipt-intent-phase-c-{action}-001",
...
def _identifiers(value: Any, key: str) -> set[str]:
    ...
    if isinstance(value, list):
        return (
            set().union(*(_identifiers(child, key) for child in value))
            if value
            else set()
        )
    return set()
...
def _case(case_id: str, observations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    uniqueness = {
        ...
        "gateway_event_count": sum(
            "gateway" in record["record_type"] for record in records
        ),
    }
    derived_sha256 = sha256_json(
        {"case_id": case_id, "timeline": timeline, **uniqueness}
    )
    return {
...
def _leader_faults() -> dict[str, Any]:
    ...
    with _raises(LeaseNotHeldError):
        LeaderFencer(repo, scope="account:phase-c").acquire(
            "standby-phase-c", now=start
        )
...
def _final_fence_stale_send_cancel() -> dict[str, Any]:
    ...
    admission = WindowsRpcFencedAdmissionV1(
        ...
        send_handler=lambda *_: calls.append("send")
        or {"state": "ACKNOWLEDGED", "accepted": True},
        cancel_handler=lambda *_: calls.append("cancel")
        or {"state": "CANCELLED", "accepted": True, "cancelled": True},
    )
...
def _timeout_no_replay() -> dict[str, Any]:
    ...
    return _case(
        ...
        [
            {
                "record_type": "durable_execution_state",
                "payload": service.repository.snapshot(),
            },
            {
                "record_type": "gateway_send_calls",
                "payload": {"calls": gateway.send_calls},
            },
        ],
    )
...
def _crash_and_restart() -> dict[str, Any]:
    ...
    assert any(
        value.get("state") == "UNKNOWN_OUTCOME"
        for value in state["send_intents"].values()
    )
    with _raises(UnknownOutcomeError):
    ...
    return _case(
        ...
        [
            {
                "record_type": "cancel_boundary_state",
                "payload": {"states": observed_cancel_states},
            },
...
def _delayed_duplicate_callback() -> dict[str, Any]:
    ...
    service._mark_intent_result(
        intent_id, {"state": "ACKNOWLEDGED", "broker_order_id": "broker-phase-c-001"}
    )
    service._mark_intent_result(
        intent_id, {"state": "ACKNOWLEDGED", "broker_order_id": "broker-phase-c-001"}
    )
...
def _custody_faults(root: Path) -> dict[str, Any]:
    schemas = {
        "phase-c-payload-v1": {
            ...
            "required": ["production", "live", "countable_forward"],
            "properties": {
    ...
    item = new_artifact_envelope(
        artifact_type="phase-c-evidence",
        trust_domain="research",
        producer_id="phase-c-harness",
        producer_version="v1",
        schema_ref="phase-c-payload-v1",
        payload={"production": False, "live": False, "countable_forward": False},
    ...
    with ArtifactCustody(
        ...
        store.publish(
            item,
            actor_id="phase-c",
            idempotency_key="publish-phase-c-001",
            correlation_id="corr-phase-c-001",
            expected_version=0,
        )
        with _raises(CustodyError):
            store.publish(
                ...
                actor_id="phase-c",
                idempotency_key="publish-phase-c-001",
                correlation_id="corr-phase-c-other",
                expected_version=1,
            )
    ...
    with ArtifactCustody(
        swap_root, writer_id="phase-c-custody", writer_epoch=1, schema_registry=schemas
    ) as store:
        store.publish(
            item,
            actor_id="phase-c",
            idempotency_key="publish-phase-c-002",
            correlation_id="corr-phase-c-002",
            expected_version=0,
        )
    ...
    return _case(
        ...
        [
            {
                "record_type": "custody_receipt_path",
                "payload": {"path": receipt_path.name},
            },
            {
                "record_type": "custody_symlink_swap",
                "payload": {"path": stored.name, "is_symlink": stored.is_symlink()},
            },
        ],
    )
...
def run_fault_acceptance(workdir: Path) -> dict[str, Any]:
    ...
    scenarios = [
        _leader_faults(),
        _final_fence_stale_send_cancel(),
        _timeout_no_replay(),
        _crash_and_restart(),
        _delayed_duplicate_callback(),
        _custody_faults(workdir),
    ]
