#!/usr/bin/env python3
"""Offline validation for the code-only C_FAST query-v4 runtime package."""
...
DEFAULT_CONTAINERFILE = ROOT / "scripts/c_fast_t1/Containerfile.query-v4"
DEFAULT_TEMPLATE = (
...
SCHEMA_SOURCES = (
    ...
    "docs/schemas/commodity-c-fast-t1-query-consume-v4.schema.json",
    "docs/schemas/commodity-c-fast-t1-query-child-started-v4.schema.json",
    "docs/schemas/commodity-c-fast-t1-query-terminal-v4.schema.json",
    "docs/schemas/commodity-c-fast-t1-query-v4-trusted-keys-v1.schema.json",
    "docs/schemas/commodity-c-fast-t1-readiness-v3.schema.json",
    (
        "docs/schemas/"
    ...
    "docs/schemas/commodity-c-fast-t1-build-registry-provenance-v2.schema.json",
    (
        "docs/schemas/"
...
FORBIDDEN_COPY_MARKERS = (
    "commodity_c_fast_t1_query_v4_sign_release.py",
...
PATH_PREFIX_BY_FLAG = {
    ...
    "--trusted-keyring": "/run/c-fast-t1-query-v4-input/",
    "--manifest": "/run/c-fast-t1-query-v4-input/",
...
EXPECTED_VOLUME_TARGETS = (
    ...
    "/run/c-fast-t1-query-v4-input/query-keyring.json",
    "/run/c-fast-t1-query-v4-input/manifest.json",
    "/run/c-fast-t1-query-v4-input/external-image-evidence.json",
    "/run/c-fast-t1-query-v4-input/source-bundle.tar",
    "/run/c-fast-t1-query-v4-input/runtime.oci.tar",
    "/run/c-fast-t1-query-v4-input/content-attestation.json",
    "/run/c-fast-t1-query-v4-input/provenance.signed.json",
    "/run/c-fast-t1-query-v4-input/provenance-keyring.json",
    "/run/c-fast-t1-query-v4-input/t1-keyring.json",
    "/run/c-fast-t1-query-v4-input/outcome-keyring.json",
    "/run/c-fast-t1-query-v4-input/l3-release.signed.json",
    "/run/c-fast-t1-query-v4-input/l3-release-keyring.json",
    "/run/c-fast-t1-query-v4-input/questdb-image-attestation.json",
    "/run/c-fast-t1-query-v4-input/readonly-principal-identity.json",
    "/run/c-fast-t1-query-v4-input/secret-file-identity.json",
    "/run/c-fast-t1-query-v4-input/writer-continuity-pre.json",
    "/run/c-fast-t1-query-v4-input/writer-continuity-post-contract.json",
    "/run/c-fast-t1-query-v4-input/health-pre.json",
    "/run/c-fast-t1-query-v4-input/backlog-pre.json",
    "/run/c-fast-t1-query-v4-input/rollback-plan.json",
    "/run/c-fast-t1-query-v4-input/root-pin-identity.json",
    "/run/c-fast-t1-query-v4-input/custody-path-identity.json",
    "/run/c-fast-t1-query-v4-input/isolated-network-attestation.json",
    "/run/c-fast-t1-query-v4-input/deployment-plan.json",
    "/run/c-fast-t1-query-v4-input/deployment-execution.json",
    "/run/c-fast-t1-query-v4-input/writer-post.json",
    "/run/c-fast-t1-query-v4-input/health-post.json",
    "/run/c-fast-t1-query-v4-input/backlog-post.json",
    "/run/c-fast-t1-query-v4-input/principal-secret-post.json",
    "/run/c-fast-t1-query-v4-input/network-post.json",
    "/run/secrets/c-fast-t1-query-v4-readonly.dsn",
    ...
    (
        "/var/lib/c-fast-t1-readiness/"
        "${C_FAST_T1_QUERY_V4_RELEASE_BASENAME:"
...
EXPECTED_METADATA = {
    ...
    "schema_version": "commodity_c_fast_t1_query_v4_runtime_template_v1",
    ...
    "requires_separate_query_v4_image_attestation": True,
    "requires_signed_query_v4_release": True,
    "requires_active_readiness_v3_pins": True,
...
class QueryV4PackagingError(RuntimeError):
...
def _read_regular(path: Path, label: str) -> bytes:
    try:
        ...
        raise QueryV4PackagingError(f"{label} is unavailable") from exc
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise QueryV4PackagingError(f"{label} must be a regular non-symlink file")
    if info.st_size > MAX_INPUT_BYTES:
        raise QueryV4PackagingError(f"{label} is too large")
    try:
        raw = path.read_bytes()
    except OSError as exc:
        raise QueryV4PackagingError(f"{label} cannot be read") from exc
    if b"\x00" in raw:
        raise QueryV4PackagingError(f"{label} contains NUL")
    return raw


def _reject_constant(value: str) -> None:
    raise QueryV4PackagingError(f"JSON constant {value!r} is forbidden")
...
def _object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise QueryV4PackagingError(f"duplicate JSON key: {key}")
        result[key] = value
    return result


def _load_template(path: Path) -> tuple[bytes, dict[str, Any]]:
    raw = _read_regular(path, "query-v4 runtime template")
    try:
        payload = json.loads(
            raw,
    ...
    except QueryV4PackagingError:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise QueryV4PackagingError("runtime template is not strict JSON") from exc
    if not isinstance(payload, dict):
        raise QueryV4PackagingError("runtime template root must be an object")
    return raw, payload
...
def _copy_map(text: str) -> dict[str, str]:
    ...
    for line in text.splitlines():
        ...
        parts = line.split()
        if len(parts) != 3:
            raise QueryV4PackagingError("COPY must use one source and one target")
        source, target = parts[1:]
        if source in result:
            raise QueryV4PackagingError("duplicate Containerfile COPY source")
...
def _validate_instruction_allowlist(text: str) -> str:
    ...
    if keywords.count("FROM") != 1:
        raise QueryV4PackagingError(
            "query-v4 Containerfile requires exactly one FROM"
        )
    if instructions[0] != f"FROM {BASE_IMAGE}":
        raise QueryV4PackagingError("query-v4 base image digest drifted")
    if any(
        keyword == "RUN"
...
def validate_containerfile(path: Path = DEFAULT_CONTAINERFILE) -> dict[str, Any]:
    raw = _read_regular(path, "query-v4 Containerfile")
    try:
        text = raw.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise QueryV4PackagingError("Containerfile is not UTF-8") from exc
    if text.count("ENTRYPOINT") != 1 or any(
        line.upper().startswith("CMD ") for line in text.splitlines()
    ):
        raise QueryV4PackagingError(
            "query-v4 image requires one ENTRYPOINT and no CMD"
        )
    ...
    if text.count(expected_entrypoint) != 1:
        raise QueryV4PackagingError("query-v4 isolated ENTRYPOINT drifted")
    for required in (
        ...
        (
            "/opt/c-fast-t1/scripts/"
            "commodity_c_fast_l1_l5_audit_v4.py"
        ),
    ):
        if text.count(required) != 1:
            raise QueryV4PackagingError(
                f"query-v4 Containerfile invariant drifted: {required}"
            )
    if text.count(
        "PYTHONPATH=/INVALID_POISON_MUST_BE_IGNORED_BY_"
    ...
    if sum(
        exact_parent_smoke in instruction
        for instruction in _logical_instructions(text)
    ) != 1:
        raise QueryV4PackagingError(
            "query-v4 exact ENTRYPOINT import closure drifted"
        )
    for dependency in REQUIRED_DEPENDENCIES:
        if text.count(f'"{dependency}"') != 1:
            raise QueryV4PackagingError(f"dependency pin drifted: {dependency}")
    copies = _copy_map(text)
    if tuple(copies) != EXPECTED_COPY_SOURCES:
        raise QueryV4PackagingError("query-v4 COPY allowlist/order drifted")
    for source, target in copies.items():
        if target != f"./{source}":
            raise QueryV4PackagingError("query-v4 COPY target drifted")
        if any(marker in source for marker in FORBIDDEN_COPY_MARKERS):
            raise QueryV4PackagingError("forbidden signing/secret source copied")
    ...
    if "docker.sock" in text or re.search(r"\b(curl|wget|ssh)\b", text):
        raise QueryV4PackagingError("runtime Containerfile adds external capability")
    if "commodity_c_fast_t1_build_registry_provenance_sign.py" in text:
        raise QueryV4PackagingError("provenance signing tool must not enter runtime")
...
def _command_pairs(command: Any) -> list[tuple[str, str]]:
    if (
        ...
        raise QueryV4PackagingError("query-v4 command shape is invalid")
    pairs = list(zip(command[::2], command[1::2]))
    if tuple(flag for flag, _value in pairs) != COMMAND_FLAGS:
        raise QueryV4PackagingError("query-v4 required command flags drifted")
    return pairs
...
def validate_runtime_template(
    ...
    if set(payload) != {
        "x-c-fast-t1-query-v4-runtime",
        "services",
        "networks",
    }:
        raise QueryV4PackagingError("runtime template top-level fields drifted")
    if payload["x-c-fast-t1-query-v4-runtime"] != EXPECTED_METADATA:
        raise QueryV4PackagingError("runtime metadata/authority boundary drifted")
    services = payload["services"]
    if not isinstance(services, dict) or set(services) != {
        "c-fast-t1-query-v4"
    }:
        raise QueryV4PackagingError("runtime service identity drifted")
    service = services["c-fast-t1-query-v4"]
    ...
    if not isinstance(service, dict) or set(service) != required_service_fields:
        raise QueryV4PackagingError("runtime service fields drifted")
    if service["entrypoint"] != ENTRYPOINT:
        raise QueryV4PackagingError("runtime entrypoint does not match Containerfile")
    if service["image"] != (
        "${C_FAST_T1_QUERY_V4_IMAGE_REPOSITORY:?required_repository}"
        "@${C_FAST_T1_QUERY_V4_IMAGE_DIGEST:?required_sha256_digest}"
    ):
        raise QueryV4PackagingError("runtime image must use an exact RepoDigest")
    pairs = _command_pairs(service["command"])
    values = dict(pairs)
    for flag, prefix in PATH_PREFIX_BY_FLAG.items():
        value = values[flag]
        if not value.startswith(prefix):
            raise QueryV4PackagingError(f"{flag} escaped its fixed mount")
    ...
    for flag in input_flags:
        if not values[flag].startswith("/run/c-fast-t1-query-v4-input/"):
            raise QueryV4PackagingError(f"{flag} escaped the readonly input bundle")
    expected_values = {
        "--expected-t1-runtime-source-commit-sha": (
            "${C_FAST_T1_QUERY_V4_SOURCE_COMMIT_SHA:"
    ...
    for flag, expected in expected_values.items():
        if values[flag] != expected:
            raise QueryV4PackagingError(f"{flag} environment binding drifted")
    fixed_service = {
        "profiles": ["manual-c-fast-t1-query-v4"],
        "networks": ["c-fast-t1-query-v4-questdb-only"],
        "read_only": True,
        "user": "65532:65532",
        "cap_drop": ["ALL"],
    ...
    for field, expected in fixed_service.items():
        if service[field] != expected:
            raise QueryV4PackagingError(f"runtime service {field} drifted")
    ...
    if not isinstance(volumes, list) or any(
        ...
        raise QueryV4PackagingError("runtime volume shape drifted")
    if tuple(volume["target"] for volume in volumes) != EXPECTED_VOLUME_TARGETS:
        raise QueryV4PackagingError("runtime exact mount targets drifted")
    ...
    for volume in volumes:
        ...
        if (
            volume["target"] == "/run/c-fast-t1-query-v4-input"
            or volume["source"].startswith("${C_FAST_T1_QUERY_V4_INPUT_DIR:")
        ):
            raise QueryV4PackagingError(
                "broad query-v4 input directory mounts are forbidden"
            )
        if (
            volume["target"] == "/run/c-fast-t1-readiness-v3-pins"
            and volume["source"]
            != (
                "${C_FAST_T1_READINESS_V3_PIN_ROOT:"
                "?required_root_owned_readiness_v3_pin_root}"
            )
        ):
            raise QueryV4PackagingError(
                "readiness-v3 must mount its exact dedicated pin root"
            )
    expected_networks = {
        "c-fast-t1-query-v4-questdb-only": {
            "external": True,
            "name": (
                "${C_FAST_T1_QUERY_V4_QUESTDB_NETWORK:"
                "?required_preapproved_isolated_network}"
            ),
        }
    }
    if payload["networks"] != expected_networks:
        raise QueryV4PackagingError("isolated query network drifted")
...
def validate_package(
    ...
    return {
        "schema_version": "commodity_c_fast_t1_query_v4_packaging_validation_v1",
        "status": "QUERY_V4_CODE_ONLY_PACKAGING_VALID_RUNTIME_BLOCKED",
...
def write_report_create_only(path: Path, payload: dict[str, Any]) -> None:
    ...
    if output.parent != parent:
        raise QueryV4PackagingError("output parent must already be normalized")
    raw = (
        json.dumps(
            payload,
            ensure_ascii=False,
    ...
    try:
        ...
        raise QueryV4PackagingError("validation output create-only write failed") from exc
    try:
...
def main() -> int:
    ...
    except (OSError, QueryV4PackagingError, ValueError) as exc:
        print(f"query-v4 packaging validation failed: {exc}", file=sys.stderr)
        return 2
    return 0
