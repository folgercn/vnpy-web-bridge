...
def run(role: str, argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (
            ...
            and options.replay_ledger_dir is None
        ):
        ...
        if schema == "windows_rpc_durable_fence_zero_order_preflight_v1":
            consume_replay_token_create_only_v1(
                options.replay_ledger_dir,
                token_sha256=str(draft["challenge_nonce_sha256"]),
            ...
            consume_replay_token_create_only_v1(
                ...
                token_sha256=hashlib.sha256(
                    str(draft["replay_guard_id"]).encode()
                ).hexdigest(),
        ...
        if schema == "windows_rpc_durable_fence_install_manifest_v1":
            consume_replay_token_create_only_v1(
                options.replay_ledger_dir,
                token_sha256=str(draft["attempt_nonce_sha256"]),
            ...
            consume_replay_token_create_only_v1(
                options.replay_ledger_dir,
                token_sha256=hashlib.sha256(
                    str(draft["install_attempt_id"]).encode()
                ).hexdigest(),
        ...
        if schema == "windows_rpc_durable_fence_restart_authorization_v1":
            consume_replay_token_create_only_v1(
                options.replay_ledger_dir,
                token_sha256=str(draft["dispatch_nonce_sha256"]),
            ...
            consume_replay_token_create_only_v1(
                options.replay_ledger_dir,
                token_sha256=hashlib.sha256(
                    str(draft["authorization_id"]).encode()
                ).hexdigest(),
        ...
        signed = sign_artifact_with_fd_v1(
            ...
            fresh_preflight_raw=(
                None
                if options.preflight_receipt is None
                else options.preflight_receipt.read_bytes()
            ),
            now=(
                None
                if options.now_utc is None
                else datetime.fromisoformat(options.now_utc.replace("Z", "+00:00"))
            ),
            execution_facts_raw=(
                None
                if options.execution_facts is None
                else options.execution_facts.read_bytes()
            ),
            snapshot_raw=(
                None if options.snapshot is None else options.snapshot.read_bytes()
            ),
        )
        raw = write_canonical_create_only_v1(options.output, signed)
        write_audit_create_only_v1(
            options.audit_output, artifact_raw=raw, action=f"sign-{role}"
        )
