...
def load_manifest_chain(
    ...
    *,
    ...
    for path in sorted(paths.manifests.rglob("batch-*.json")):
        ...
        expected = (
            paths.manifests / payload["trade_day"] / f"{payload['batch_id']}.json"
        )
        if path != expected:
        ...
        enriched["commit_seal_sha256"] = (
            loaded_receipt[1] if loaded_receipt is not None else None
        )
    ...
    roots = [item for item in manifests if item["parent_batch_seal_sha256"] is None]
    if len(roots) != 1:
    ...
    while ordered[-1]["batch_seal_sha256"] in children:
        ...
        if child["parent_commit_seal_sha256"] != ordered[-1]["commit_seal_sha256"]:
            raise RegistryError("manifest parent commit seal mismatch")
    ...
    uncommitted = [
        index for index, item in enumerate(ordered) if item["commit_receipt"] is None
    ]
    ...
    *,
    ...
    if not chain:
        if (
            ...
            or expected_head_commit_seal_sha256 is not None
        ):
    ...
    for label, value in (
        ...
        ("head commit", expected_head_commit_seal_sha256),
    ):
    ...
    if chain[-1]["commit_seal_sha256"] != expected_head_commit_seal_sha256:
        raise RegistryError("manifest head commit does not match trusted anchor")
