...
class DirectoryProjectionSource:
    def __init__(
        self, directory: str | Path | None, *, max_age_seconds: float = 60.0
    ) -> None:
...
class TelegramNotifier:
    def __init__(
        self, token: str, chat_id: str, *, timeout_seconds: float = 5.0
    ) -> None:
        ...
        payload = urlencode(
            {
                ...
                "text": f"[{incident.severity}] {incident.service_id}: {incident.summary}",
            }
        ).encode()
        request = Request(
...
class SqliteIncidentOutbox:
    ...
    def activate_generation(self, generation: str) -> int:
        with self._connect() as db:
            ...
            row = db.execute(
                "SELECT generation,epoch FROM worker_fence WHERE singleton=1"
            ).fetchone()
            if row is None:
                epoch = 1
                db.execute(
                    "INSERT INTO worker_fence VALUES (1,?,?)", (generation, epoch)
                )
                ...
                epoch = int(row[1]) + 1
                db.execute(
                    "UPDATE worker_fence SET generation=?,epoch=? WHERE singleton=1",
                    (generation, epoch),
                )
            return epoch
    ...
    def record(self, incident: IncidentEvent) -> bool:
        ...
        with self._connect() as db:
            ...
            if inserted:
                db.execute(
                    "INSERT INTO delivery_outbox (incident_id,payload_json) VALUES (?,?)",
    ...
    def pending(self, *, limit: int = 100) -> list[dict[str, object]]:
        with self._connect() as db:
            rows = db.execute(
                "SELECT delivery_id,incident_id,payload_json,attempts FROM delivery_outbox WHERE state='pending' ORDER BY delivery_id LIMIT ?",
                (max(1, int(limit)),),
            ).fetchall()
        return [
            {
    ...
    def mark_delivered(self, delivery_id: int, *, generation: str, epoch: int) -> bool:
        with self._connect() as db:
            return (
                db.execute(
                    "UPDATE delivery_outbox SET state='delivered',lease_generation=?,delivered_at_utc=? WHERE delivery_id=? AND state='pending' AND EXISTS (SELECT 1 FROM worker_fence WHERE singleton=1 AND generation=? AND epoch=?)",
                    (generation, isoformat(), int(delivery_id), generation, int(epoch)),
                ).rowcount
                == 1
            )
...
class MonitorConfig:
    ...
    def from_environment(cls, state_dir: str | Path | None = None) -> MonitorConfig:
        root = Path(
            state_dir
            or os.getenv("PHASE_B_MONITOR_STATE_DIR", "/var/lib/phase-b/monitor")
        )
        enabled = os.getenv("PHASE_B_TELEGRAM_ENABLED", "false").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
        egress_permitted = os.getenv(
            "PHASE_B_TELEGRAM_EGRESS_PERMITTED", "false"
        ...
        return cls(
            ...
            os.getenv("SOURCE_REVISION", "unknown"),
            os.getenv("PHASE_B_MONITOR_GENERATION", "generation-1"),
            enabled,
            egress_permitted,
        )
...
class JsonMonitorStateRepository:
    def __init__(self, path: str | Path) -> None:
        self.checkpoint = AtomicCheckpoint(
            path, default={"episodes": {}, "delivery_pending": {}}
        )

    def read(self) -> dict[str, object]:
        return self.checkpoint.read()

    def write(self, value: Mapping[str, object]) -> None:
        self.checkpoint.write(value)
...
class MonitorWorker:
    ...
    def __init__(
        self,
        config: MonitorConfig | str | Path,
        *,
        generation: str | None = None,
        source: ProjectionSource | None = None,
        notifier: AlertNotifier | None = None,
        state: MonitorStateRepository | None = None,
        identity: WorkerIdentity | None = None,
    ) -> None:
        if not isinstance(config, MonitorConfig):
            root = Path(config)
            config = MonitorConfig(
                root, root / "projections", generation=generation or "generation-1"
            )
        self.config = config
        ...
        token, chat = (
            os.getenv("PHASE_B_TELEGRAM_TOKEN", ""),
            os.getenv("PHASE_B_TELEGRAM_CHAT_ID", ""),
        )
        self.notifier = notifier or (
            TelegramNotifier(token, chat)
            if config.telegram_enabled
            and config.telegram_egress_permitted
            and token
            and chat
            else NullNotifier(reason="network_disabled_or_not_configured")
        )
        self.state = state or JsonMonitorStateRepository(
            config.state_dir / "monitor_state.json"
        )
        ...
        self.metrics = WorkerMetrics(
            self.service_id, isoformat(), worker_generation=config.generation
        )
    ...
    def _deliver_outbox(self) -> None:
        ...
        for item in self.repository.pending():
            ...
            try:
                ...
                self._last_error = type(exc).__name__
                continue
            if delivered and self.repository.mark_delivered(
    ...
    def observe(
        self, service_id: str, projection: Mapping[str, object]
    ) -> IncidentEvent | None:
        if not self.fence_epoch:
            self.recover()
        ...
        if (
            status in {"ok", "live", "ready", "healthy", "nominal"}
            and safe.get("ready") is not False
        ):
            return None
        incident = IncidentEvent.create(
            service_id=str(service_id),
            episode_key=f"{service_id}:unavailable",
            severity="critical"
            if status in {"failed", "unavailable", "unhealthy"}
            else "warning",
            state="open",
            summary=f"typed projection reports {status}",
            source_revision=self.identity.source_revision,
            details={"projection": safe},
        )
        self.metrics.increment(
            "incidents_created"
            if self.repository.record(incident)
            else "incidents_deduplicated"
        )
    ...
    def run_once(self) -> dict[str, object]:
        with self._lock:
            ...
            episodes = (
                durable.get("episodes")
                if isinstance(durable.get("episodes"), dict)
                else {}
            )
            for projection in projections:
                ...
                status = str(safe.get("status") or "unknown").lower()
                healthy = (
                    status in {"healthy", "ready", "ok", "nominal"}
                    and safe.get("ready", True) is not False
                )
                ...
                if not isinstance(prior, Mapping) or prior.get("state") != state:
                    incident = IncidentEvent.create(
                        ...
                        episode_key=key,
                        severity="critical"
                        if not healthy and safe.get("ready") is False
                        else "warning",
                        state=state,
                        summary="worker healthy"
                        if healthy
                        else f"worker unhealthy ({status})",
                    ...
                    if isinstance(episodes, dict):
                        episodes[key] = {
                            ...
                            "updated_at_utc": incident.occurred_at_utc,
                        }
    ...
    def health(self) -> HealthSnapshot:
        ...
        notifier = (
            ...
            if hasattr(self.notifier, "status")
        ...
        return HealthSnapshot(
            ...
            {
                "projection_source": {
                    "status": "healthy" if projection[0] else "unavailable",
                    "reason": projection[1],
                },
                "notifier": notifier,
                "incident_state": {"status": "healthy"},
            },
            self._last_error,
        )
    ...
    def readiness(self) -> ReadinessSnapshot:
        ...
        source_ready, reason = DirectoryProjectionSource(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.version:
        ...
        value = {"snapshot": worker.run_once(), "metrics": worker.metrics_snapshot()}
    elif args.run:
        ...
        worker.run(
            stop_event=stop,
            interval_seconds=float(os.getenv("PHASE_B_MONITOR_INTERVAL_SECONDS", "5")),
        )
        return 0
    else:
