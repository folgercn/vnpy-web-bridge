...
QUERY_V4_PIN_ROOT = Path("/run/c-fast-t1-readiness-v3-pins")
QUERY_V4_PIN_NAMES = {
    ...
    "query_v4_authority_keyring_sha256": (
        "query-v4-authority-keyring.sha256"
    ),
...
QUERY_V4_READINESS_PATH_INPUTS = frozenset(
    {
...
def _verify_query_v4_signed_release(
    gate: dict[str, Any],
    release: dict[str, Any],
    readiness: dict[str, Any],
) -> None:
    try:
        keyring = _verify_query_gate_json_binding(
            Path(gate["query_v4_keyring_path"]),
            gate["query_v4_keyring_raw_sha256"],
            gate["query_v4_keyring_canonical_sha256"],
            "query-v4 trusted keyring",
        )
        validate_json_schema(
            release,
            QUERY_V4_RELEASE_SCHEMA_PATH,
            "query-v4 signed release",
        )
        validate_json_schema(
            keyring,
            QUERY_V4_KEYRING_SCHEMA_PATH,
        ...
        validate_json_schema(
            readiness,
            QUERY_V4_READINESS_SCHEMA_PATH,
            "query-v4 readiness-v3",
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise AuditError(
            "query-v4 signed release inputs are invalid"
        ) from exc
    schema_bindings = (
        (
            "release_schema_sha256",
            QUERY_V4_RELEASE_SCHEMA_PATH,
            "query-v4 release schema",
        ),
        (
            "query_keyring_schema_sha256",
            QUERY_V4_KEYRING_SCHEMA_PATH,
            "query-v4 keyring schema",
        ),
        (
            "readiness_schema_sha256",
            QUERY_V4_READINESS_SCHEMA_PATH,
            "query-v4 readiness schema",
        ),
    )
    try:
        for field_name, schema_path, label in schema_bindings:
            if not hmac.compare_digest(
                hashlib.sha256(
                    _read_query_gate_regular_bytes(schema_path, label)
                ).hexdigest(),
                release[field_name],
            ):
                raise AuditError(
                    "query-v4 signed schema binding changed before connect"
                )
    except (KeyError, TypeError) as exc:
        raise AuditError(
            "query-v4 signed schema binding is invalid"
        ) from exc
    active_query_keyring_sha256 = _read_query_gate_root_pin(
        QUERY_V4_PIN_ROOT
        / QUERY_V4_PIN_NAMES["query_v4_authority_keyring_sha256"],
        "query_v4_authority_keyring_sha256",
    )
    if (
        not isinstance(keyring, dict)
        or set(keyring) != {"schema_version", "keys"}
        or keyring.get("schema_version")
        ...
        or not hmac.compare_digest(
            ...
            gate["query_v4_authority_keyring_sha256"],
        )
        or not hmac.compare_digest(
            gate["query_v4_keyring_canonical_sha256"],
            gate["query_v4_authority_keyring_sha256"],
        ...
        or not hmac.compare_digest(
            gate["query_v4_keyring_canonical_sha256"],
            active_query_keyring_sha256,
        )
        or not hmac.compare_digest(
            str(release.get("trusted_keyring_sha256", "")),
            gate["query_v4_authority_keyring_sha256"],
        )
    ):
        raise AuditError(
            "query-v4 trusted keyring does not match the active pin"
        )
    signer_key_id = str(release.get("signer_key_id", ""))
    signer_material: bytes | None = None
    ...
    for entry in keyring["keys"]:
        if (
            ...
            or set(entry)
            != {"key_id", "purpose", "public_key_base64"}
        ):
            raise AuditError("query-v4 trusted keyring entry is invalid")
        key_id = str(entry["key_id"])
        if key_id in seen_ids:
            raise AuditError(
                "query-v4 trusted keyring contains duplicate key IDs"
            )
        seen_ids.add(key_id)
        try:
            ...
            raise AuditError(
                "query-v4 trusted keyring public key is invalid"
            ) from exc
        if public_raw in seen_materials:
            raise AuditError(
                "query-v4 trusted keyring reuses public-key material"
            )
        seen_materials.add(public_raw)
        if key_id == signer_key_id:
            if entry["purpose"] != QUERY_V4_KEY_PURPOSE:
                raise AuditError(
                    "query-v4 release signer purpose is not authorized"
                )
            signer_material = public_raw
    if signer_material is None:
        raise AuditError("query-v4 release signer is not trusted")
    unsigned_release = {
    ...
    try:
        ...
        Ed25519PublicKey.from_public_bytes(signer_material).verify(
            signature,
            json.dumps(
                unsigned_release,
                sort_keys=True,
        ...
        raise AuditError(
            "query-v4 release Ed25519 signature is invalid"
        ) from exc
    signer_sha256 = hashlib.sha256(signer_material).hexdigest()
    release_readiness = release.get("readiness")
    runtime = readiness.get("t1_runtime")
    provenance = readiness.get("build_registry_provenance")
    outcome = readiness.get("readonly_deployment_outcome")
    if not all(
        isinstance(value, dict)
        for value in (release_readiness, runtime, provenance, outcome)
    ):
        raise AuditError("query-v4 readiness binding is invalid")
    expected_readiness_binding = {
        "packet_id": readiness["packet_id"],
        "packet_raw_sha256": gate["readiness_raw_sha256"],
        "packet_canonical_sha256": gate["readiness_canonical_sha256"],
    ...
    source_bundle_index = {
        ...
        "t1_runtime": runtime,
        "build_registry_provenance": provenance,
        "readonly_deployment_outcome": outcome,
    }
    namespaces = {
    ...
    if (
        release_readiness != expected_readiness_binding
        or release["attempt_id"]
        != "attempt-"
        + hashlib.sha256(
            release["release_id"].encode("utf-8")
        ).hexdigest()
        or release["custody_identity_sha256"]
        != readiness["packet_custody_identity_sha256"]
        or release["custody_identity_sha256"]
        != gate["packet_custody_identity_sha256"]
        or release["pin_root_path_sha256"]
        != readiness["pin_root_path_sha256"]
        ...
        or release["namespaces"] != namespaces
        or signer_sha256
        in {
            release_readiness["provenance_signer_public_key_sha256"],
            release_readiness["outcome_signer_public_key_sha256"],
        }
        or not str(release.get("human_signature", "")).strip()
        or str(release.get("human_signature", "")).startswith("PENDING_")
        or not str(release.get("reviewer_role", "")).strip()
        or str(release.get("reviewer_role", "")).startswith("PENDING_")
    ):
        raise AuditError(
            "query-v4 signed release/readiness binding is invalid"
        )


def _verify_query_v4_full_readiness(
    gate: dict[str, Any],
    release: dict[str, Any],
    readiness: dict[str, Any],
    *,
    ...
    try:
        query_v4_readiness = importlib.import_module(
            "commodity_c_fast_t1_readiness_v3"
        )
    except ImportError as exc:
        raise AuditError(
            "query-v4 readiness verifier cannot be loaded"
        ) from exc
    values = gate.get("readiness_verification_inputs")
    if (
        not isinstance(values, dict)
        or set(values)
        != QUERY_V4_READINESS_PATH_INPUTS
        | QUERY_V4_READINESS_SCALAR_INPUTS
        or any(
            not isinstance(value, str) or not value
            for value in values.values()
        )
        or not isinstance(gate.get("source_readiness_path"), str)
        or not gate["source_readiness_path"]
    ):
        raise AuditError(
            "query-v4 full readiness inputs are invalid"
        )
    try:
        readiness_source = Path(query_v4_readiness.__file__).resolve(
            strict=True
        )
        if not hmac.compare_digest(
            hashlib.sha256(
                _read_query_gate_regular_bytes(
                    readiness_source,
                    "query-v4 readiness verifier",
                )
            ).hexdigest(),
            release["readiness_verifier_sha256"],
        ):
            raise AuditError(
                "query-v4 readiness verifier changed before connect"
            )
        namespace = argparse.Namespace(
            **{
                key: (
                    Path(value)
                    if key in QUERY_V4_READINESS_PATH_INPUTS
                    else value
                )
                for key, value in values.items()
            }
        )
        inputs = query_v4_readiness.inputs_from_args(namespace)
        pins = query_v4_readiness.ReadinessPins(
            pin_set_generation_id=gate["pin_set_generation_id"],
            pin_set_manifest_sha256=gate["pin_set_manifest_sha256"],
            pin_root_identity_sha256=gate["pin_root_identity_sha256"],
            provenance_keyring_sha256=gate[
                "provenance_keyring_sha256"
            ],
            provenance_signing_tool_source_sha256=gate[
                "provenance_signing_tool_source_sha256"
            ],
            provenance_signing_tool_source_commit_sha=gate[
                "provenance_signing_tool_source_commit_sha"
            ],
            t1_authority_keyring_sha256=gate[
                "t1_authority_keyring_sha256"
            ],
            l3_authority_keyring_sha256=gate[
                "l3_authority_keyring_sha256"
            ],
            outcome_keyring_sha256=gate["outcome_keyring_sha256"],
            packet_custody_path=Path(gate["packet_custody_path"]),
            packet_custody_id=gate["packet_custody_id"],
            packet_custody_identity_sha256=gate[
                "packet_custody_identity_sha256"
            ],
            packet_custody_directory_identity_sha256=gate[
                "packet_custody_directory_identity_sha256"
            ],
            evidence_join_identity_sha256=gate[
                "evidence_join_identity_sha256"
            ],
        )
        verified = query_v4_readiness.verify_existing_readiness_packet(
            inputs,
            pins,
            Path(gate["source_readiness_path"]),
            now=now,
        ...
        TypeError,
        ValueError,
        query_v4_readiness.ReadinessV3Error,
    ) as exc:
        raise AuditError(
            "query-v4 full readiness-v3 revalidation failed"
        ) from exc
    if (
        verified.payload != readiness
        or not hmac.compare_digest(
            verified.raw_sha256,
        ...
        raise AuditError(
            "query-v4 full readiness-v3 result changed before connect"
        )


def _verify_query_v4_launch_capability(
    gate: dict[str, Any],
    release: dict[str, Any],
    custody: Path,
    *,
    ...
    child_raw = _read_query_gate_regular_bytes(
        child_path,
        "frozen query-v4 bootstrap",
    )
    if not hmac.compare_digest(
        hashlib.sha256(child_raw).hexdigest(),
        str(release.get("query_child_sha256", "")),
    ):
        raise AuditError("frozen query-v4 bootstrap changed before connect")
    child_module = ModuleType("_frozen_commodity_c_fast_t1_query_child_v4")
    ...
    try:
        ...
        raise AuditError(
            "cannot load the frozen query-v4 launch verifier"
        ) from exc
    try:
        child_module.verify_active_pins(
            {
                "pin_set_generation_id": gate["pin_set_generation_id"],
                "pin_set_manifest_sha256": gate[
                ...
                "query_v4": gate["query_v4_authority_keyring_sha256"],
                "l3_authority_keyring_sha256": gate[
                    "l3_authority_keyring_sha256"
                ],
                "outcome_keyring_sha256": gate[
                    "outcome_keyring_sha256"
                ],
                "packet_custody_path": gate["packet_custody_path"],
                "packet_custody_id": gate["packet_custody_id"],
                "packet_custody_identity_sha256": gate[
                    "packet_custody_identity_sha256"
                ],
        ...
        child_module.verify_query_child_launch_capability(
            ...
            parent_launch_capability_sha256=(
                gate["parent_launch_capability_sha256"]
            ),
        )
    except (KeyError, TypeError, child_module.QueryChildError) as exc:
        raise AuditError(
            "query-v4 launch capability is invalid before connect"
        ) from exc


def _verify_query_v4_authority_and_launch(
    gate: dict[str, Any],
    release: dict[str, Any],
    readiness: dict[str, Any],
    custody: Path,
    *,
    ...
    _verify_query_v4_signed_release(gate, release, readiness)
    _verify_query_v4_full_readiness(
        gate,
        release,
        readiness,
        now=now,
...
def verify_pre_connect_query_gate(
    ...
    *,
    ...
    """Perform the query-v4 last gate immediately before DSN access."""
    gate_raw = _read_query_gate_regular_bytes(
        gate_path,
        "query-v4 pre-connect gate",
    )
    if not hmac.compare_digest(
        ...
        raise AuditError(
            "query-v4 pre-connect gate exact raw binding changed"
        )
    ...
    if not hmac.compare_digest(
        ...
        raise AuditError(
            "query-v4 pre-connect gate canonical binding changed"
        )
    required = {
        "schema_version",
        "purpose",
        ...
        "query_child_invocation_path",
        "query_v4_keyring_path",
        "query_v4_keyring_raw_sha256",
        "query_v4_keyring_canonical_sha256",
    ...
    if not isinstance(gate, dict) or set(gate) != required:
        raise AuditError("query-v4 pre-connect gate fields are invalid")
    if (
        gate["schema_version"] != "commodity_c_fast_t1_pre_connect_gate_v4"
        or gate["purpose"] != "c_fast_t1_last_active_pin_gate_before_dsn"
        or not isinstance(gate["audit_invocation_path"], str)
        or not gate["audit_invocation_path"]
        or not isinstance(gate["query_child_invocation_path"], str)
        or not gate["query_child_invocation_path"]
        or not isinstance(gate["query_v4_keyring_path"], str)
        or not gate["query_v4_keyring_path"]
        or not isinstance(gate["source_readiness_path"], str)
        or not gate["source_readiness_path"]
        or not isinstance(gate["readiness_verification_inputs"], dict)
        or any(
            not isinstance(gate[field], str)
            or re.fullmatch(r"[0-9a-f]{64}", gate[field]) is None
            for field in (
                "query_v4_keyring_raw_sha256",
                "query_v4_keyring_canonical_sha256",
            )
        )
        or not isinstance(gate["parent_launch_capability_sha256"], str)
        or re.fullmatch(
            r"[0-9a-f]{64}",
            gate["parent_launch_capability_sha256"],
        )
        ...
        raise AuditError("query-v4 pre-connect gate identity is invalid")
    if not hmac.compare_digest(
        hashlib.sha256(
            _read_query_gate_regular_bytes(
                Path(__file__),
    ...
    if actual_invocation[-len(expected_suffix) :] != expected_suffix:
        raise AuditError(
            "query-v4 invocation gate expectations are not bootstrap-bound"
        )
    ...
    if (
        ...
        != "commodity_c_fast_t1_one_shot_query_release_v4"
        or release.get("purpose")
        != "c_fast_t1_exact_readiness_readonly_query_authority_v4"
        ...
        != gate["readiness_canonical_sha256"]
        or readiness.get("pin_set_generation_id")
        != gate["pin_set_generation_id"]
        or readiness.get("pin_set_manifest_sha256")
        != gate["pin_set_manifest_sha256"]
        or readiness.get("pin_root_identity_sha256")
        != gate["pin_root_identity_sha256"]
        or readiness.get("packet_custody_path_sha256")
        != hashlib.sha256(
            gate["packet_custody_path"].encode("utf-8")
        ).hexdigest()
        or readiness.get("packet_custody_id")
        != gate["packet_custody_id"]
        or readiness.get("packet_custody_identity_sha256")
        != gate["packet_custody_identity_sha256"]
        or readiness.get("packet_custody_directory_identity_sha256")
        != gate["packet_custody_directory_identity_sha256"]
        or readiness.get("evidence_join_identity_sha256")
        != gate["evidence_join_identity_sha256"]
    ):
        raise AuditError("query-v4 release gate bindings are inconsistent")
    current = datetime.now(timezone.utc) if now is None else now
    if current.tzinfo is None or current.utcoffset() is None:
        raise AuditError("query-v4 final gate time must be timezone-aware")
    current = current.astimezone(timezone.utc)
    try:
        issued_at = _parse_cli_datetime(
            str(release["issued_at"]),
            "query-v4 release issued_at",
        )
        not_before = _parse_cli_datetime(
            str(release["not_before"]),
            "query-v4 release not_before",
        )
        release_expires_at = _parse_cli_datetime(
            str(release["expires_at"]),
            "query-v4 release expires_at",
        )
        readiness_generated_at = _parse_cli_datetime(
            str(readiness["generated_at"]),
            "query-v4 readiness generated_at",
        )
        readiness_expires_at = _parse_cli_datetime(
            str(readiness["expires_at"]),
            "query-v4 readiness expires_at",
        )
    except (KeyError, TypeError) as exc:
        raise AuditError(
            "query-v4 final gate lifecycle fields are invalid"
        ) from exc
    ...
    if (
        ...
        or release_expires_at - issued_at > timedelta(minutes=10)
        or release_expires_at > readiness_expires_at
        or current
        + timedelta(
            seconds=int(release["minimum_launch_margin_seconds"])
        )
        >= release_expires_at
    ):
        raise AuditError(
            "query-v4 release/readiness is not active at final query boundary"
        )
    try:
        ...
        raise AuditError(
            "query-v4 invocation lacks manifest expectation"
        ) from exc
    if not hmac.compare_digest(
        invocation_manifest_sha256,
        gate["manifest_canonical_sha256"],
    ):
        raise AuditError("query-v4 manifest gate is not the invoked manifest")
    try:
        root_info = QUERY_V4_PIN_ROOT.lstat()
    except OSError as exc:
        raise AuditError("query-v4 active pin root is unavailable") from exc
    if (
        ...
        raise AuditError("query-v4 active pin root metadata is unsafe")
    pin_root_identity = {
        "resolved_path": str(QUERY_V4_PIN_ROOT.resolve(strict=True)),
        "device": root_info.st_dev,
        "inode": root_info.st_ino,
        "owner_uid": root_info.st_uid,
        "mode": stat.S_IMODE(root_info.st_mode),
        "file_type": stat.S_IFMT(root_info.st_mode),
    }
    if not hmac.compare_digest(
        hashlib.sha256(
            json.dumps(
        ...
        raise AuditError(
            "readiness-v3 pin root identity changed at final query boundary"
        )
    observed = {
        pin_field: _read_query_gate_root_pin(
            QUERY_V4_PIN_ROOT / filename,
            pin_field,
        )
        for pin_field, filename in QUERY_V4_PIN_NAMES.items()
    }
    for pin_field in (
        "provenance_keyring_sha256",
        "provenance_signing_tool_source_sha256",
    ...
    try:
        ...
        raise AuditError("query-v4 active custody cannot be resolved") from exc
    if observed_custody != expected_custody:
        raise AuditError("active custody changed at the final query boundary")
    _verify_query_v4_authority_and_launch(
        gate,
        release,
        readiness,
        expected_custody,
        ...
        pre_connect_gate_raw_sha256=expected_gate_raw_sha256,
        pre_connect_gate_canonical_sha256=(
            expected_gate_canonical_sha256
        ),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "required query-v4 exact-binding and active-pin gate; "
    ...
    parser.add_argument(
        "--expected-pre-connect-gate-raw-sha256",
        required=True,
        help="query-v4 create-only gate raw-byte SHA256",
    )
    parser.add_argument(
        "--expected-pre-connect-gate-canonical-sha256",
        required=True,
        help="query-v4 create-only gate canonical JSON SHA256",
    )
...
def validate_artifact_paths(args: argparse.Namespace) -> None:
    input_paths = {
        ...
        args.pre_connect_query_gate.expanduser().resolve(),
    }
    raw_outputs = (
        args.json_output,
        args.csv_output,
