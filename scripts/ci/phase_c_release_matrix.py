...
def _unit(phase: str, unit: str, verification_units: list[str], source_sha: str) -> dict[str, Any]:
...
def create_plan(
    paths: list[str], *, source_commit_sha: str, baseline_known: bool = True, force_all: bool = False
) -> dict[str, Any]:
    ...
    if not baseline_known:
        blocked_reasons.append({"phase": "C", "path": "<baseline>", "code": "unknown_baseline", "rule_ids": []})
    if not changed_paths and not force_all:
        blocked_reasons.append({"phase": "C", "path": "<empty>", "code": "empty_change_set", "rule_ids": []})
    ...
    for unit in phase_a["dependency_closure"]:
        units.append(_unit("A", str(unit), list(phase_a["verification_units"]), source_commit_sha))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--baseline-status", choices=("known", "unknown"), default="known")
    ...
    Draft202012Validator(json.loads(PLAN_SCHEMA_PATH.read_text(encoding="utf-8"))).validate(plan)
    ...
    if args.github_output:
        ...
        with Path(args.github_output).open("a", encoding="utf-8") as stream:
            stream.write(f"build_required={'true' if plan['decision'] == 'BUILD_ONLY' else 'false'}\n")
