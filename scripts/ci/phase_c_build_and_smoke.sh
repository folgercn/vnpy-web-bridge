...
case "$smoke_profile" in
  ...
  gateway-request-proxy|gateway-publish-proxy)
    ...
    docker image inspect "$image" | python -c "import json,sys; c=json.load(sys.stdin)[0]['Config']; assert c['Entrypoint'] == ['python','/usr/local/bin/gateway_proxy.py']; assert c['Cmd'] == ['$expected']; assert c['User'] == '65532:65532'"
