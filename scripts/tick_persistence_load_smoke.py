...
def build_settings(*, count: int, batch_size: int, spool_dir: str | None) -> Settings:
    queue_size = max(count + 1, int(os.getenv("QUESTDB_TICK_QUEUE_SIZE", "100000")))
    return Settings(
        app_env="development",
...
def main() -> int:
    ...
    settings = build_settings(count=args.count, batch_size=args.batch_size, spool_dir=args.spool_dir)
