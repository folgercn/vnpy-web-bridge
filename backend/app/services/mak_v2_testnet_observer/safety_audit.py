...
class MakV2SafetyAuditService:
    ...
    def audit(self, payload: MakV2SafetyAuditRequestDTO, observer_status: dict[str, Any]) -> dict[str, Any]:
        ...
        checks.append(_check("observer_enabled", observer_status.get("enabled") is True, observer_status.get("enabled")))
        checks.append(_check("manual_approval_active", observer_status.get("manual_approval") is True, observer_status.get("manual_approval")))
        checks.append(_check("testnet_mode_active", observer_status.get("testnet_mode") is True, observer_status.get("testnet_mode")))
