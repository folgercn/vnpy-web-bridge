...
class PhaseCExecutionService:
    ...
    def _remote_receipt(self, receipt_id: str) -> dict[str, Any] | None:
        try:
            with httpx.Client(
                ...
                response = client.get(
                    f"{self.settings.custody_url.rstrip('/')}/internal/v1/receipts/{receipt_id}"
                )
        except httpx.HTTPError as exc:
            raise WorkflowAdapterError(
                "custody receipt lookup outcome unknown"
            ) from exc
        if response.status_code == 404:
            return None
        if response.status_code != 200:
            raise WorkflowAdapterError("custody receipt lookup rejected")
        body = response.json()
    ...
    def _load(self) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                or envelope["schema_version"] != "phase-c-execution-state-v1"
            ):
                raise ValueError
            payload = envelope["payload"]
            if (
                not isinstance(payload, dict)
                or hashlib.sha256(canonical_json(payload)).hexdigest()
            ...
            if (
                ...
                or payload["requested_state"]
                not in {"DISABLED", "ENABLE_REQUESTED", "REVOKED"}
                or not isinstance(payload["commands"], dict)
                or not isinstance(payload["audit"], list)
                or not isinstance(payload["archive"], list)
            ):
                raise ValueError
            return payload
        except (
            ...
            raise WorkflowAdapterError(
                "phase-c execution durable state is invalid"
            ) from exc
    ...
    def _save(self, value: dict[str, Any]) -> None:
        ...
        raw = canonical_json(
            {
                "schema_version": "phase-c-execution-state-v1",
                "payload": value,
                "payload_sha256": hashlib.sha256(payload).hexdigest(),
            }
        )
        fd, name = tempfile.mkstemp(
            prefix=".phase-c-execution-", dir=self.settings.state_path.parent
        )
        try:
            view = memoryview(raw)
            while view:
    ...
    @contextmanager
    def _locked(self):
        lock_path = self.settings.state_path.with_suffix(
            self.settings.state_path.suffix + ".lock"
        )
    ...
    def command(
        self, request: AuthorizationCommandDTO, *, receipt: dict[str, Any]
    ) -> AuthorizationStatusDTO:
        with self._locked():
    ...
    def _command_locked(
        ...
        if old:
            if old["fingerprint"] != fingerprint:
                raise IdempotencyConflictError("authorization idempotency conflict")
            return AuthorizationStatusDTO.model_validate(old["status"])
        if request.expected_version != state["version"]:
            raise ExpectedVersionError("authorization expected version conflict")
        required_receipt = {
            "receipt_id",
            "receipt_type",
            "artifact_id",
        ...
        if (
            ...
            or receipt.get("receipt_id") != request.custody_receipt_id
            or receipt.get("artifact_id") != request.authorization_artifact_id
            or receipt.get("receipt_type") != "install"
            or receipt.get("artifact_type") != "runtime-authorization"
            or receipt.get("trust_domain") != "runtime_authorization"
            or receipt.get("schema_ref") != "phase-c-runtime-authorization-v1"
            or not isinstance(expires_at, datetime)
            or expires_at.tzinfo is None
            or expires_at <= datetime.now(timezone.utc)
            or receipt.get("scope") != self.settings.allowed_scope
            or any(
            ...
            raise WorkflowAdapterError(
                "execution command lacks a verified custody install receipt"
            )
        state["version"] += 1
        state["requested_state"] = (
            "ENABLE_REQUESTED" if request.action == "enable" else "REVOKED"
        )
        state["artifact_id"] = request.authorization_artifact_id
        state["receipt_id"] = request.custody_receipt_id
        ...
        state["commands"][request.idempotency_key] = {
            "fingerprint": fingerprint,
            "status": status,
        }
        state["audit"].append(event)
        state["archive"].append({"kind": "authorization-command", **event})
        self._save(state)
    ...
    def receipt(self, key: str) -> AuthorizationStatusDTO | None:
        found = self._load()["commands"].get(key)
        return AuthorizationStatusDTO.model_validate(found["status"]) if found else None

    def projection(self) -> ExecutionProjectionDTO:
        state = self._load()
        return ExecutionProjectionDTO(
            status="ARCHIVED" if state["archive"] else "OFFLINE",
            audit=state["audit"],
...
def create_app(service: PhaseCExecutionService | None = None) -> FastAPI:
    if service is None:
        raise RuntimeError("Phase C execution requires explicit settings")
    app = FastAPI(title="Phase C Execution", docs_url=None, redoc_url=None)

    def auth(request: Request) -> None:
        if (
            not hmac.compare_digest(
                request.headers.get("X-Phase-C-Execution-Secret", ""),
                service.settings.secret,
            )
            or request.headers.get("X-Phase-C-Principal") != "control-api"
        ):
            raise HTTPException(401, "execution authentication failed")
    ...
    def command(payload: AuthorizationCommandDTO, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            if receipt_value is None:
                raise WorkflowAdapterError("custody receipt not found")
            return service.command(payload, receipt=receipt_value).model_dump(
                mode="json"
            )
    ...
    def projection(request: Request) -> dict[str, Any]:
        auth(request)
        return service.projection().model_dump(mode="json")
