...
def test_registry_mutations_fail_closed(
    ...
    with pytest.raises(RegistryError, match=match):
        load_registry(write_payload(tmp_path, value))
...
def test_research_package_has_no_execution_imports() -> None:
    assert_research_source_boundary(
        list((ROOT / "scripts/research_warehouse").glob("*.py"))
    )
...
def test_boundary_rejects_execution_environment_read(tmp_path: Path) -> None:
    source = tmp_path / "bad_env.py"
    source.write_text(
        'import os\nvalue = os.getenv("WEB_TRADE_ENABLED")\n',
        encoding="utf-8",
    )

    with pytest.raises(RegistryError, match="forbidden execution environment"):
        assert_research_source_boundary([source])
