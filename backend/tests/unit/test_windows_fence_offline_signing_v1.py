...
from scripts.windows_fence_foundation.contracts import AUTHORITY_FIELDS, canonical_json_bytes
...
def _facts() -> bytes:
    return canonical_json_bytes({"pending_send_outcomes": 0, "active_orders": [], "positions": []})


def _key_fd(tmp_path: Path, private: Ed25519PrivateKey) -> int:
    path = tmp_path / "offline-key"
    path.write_bytes(base64.b64encode(private.private_bytes(serialization.Encoding.Raw, serialization.PrivateFormat.Raw, serialization.NoEncryption())))
    path.chmod(0o400)
    return os.open(path, os.O_RDONLY)


@pytest.mark.skipif(os.name == "nt", reason="offline signing requires a provably read-only FD")
def test_fd_only_sign_verify_and_create_only_audit(tmp_path: Path) -> None:
    ...
    assert require_fresh_zero_preflight_v1(raw, pin=pin, now=datetime(2026, 8, 9, 0, 0, 10, tzinfo=timezone.utc)).value["active_orders"] == []
...
@pytest.mark.parametrize("field,value", [("pending_send_outcomes", 1), ("active_orders", [{"id": "CTP.1"}]), ("web_trade_enabled", True)])
@pytest.mark.skipif(os.name == "nt", reason="offline signing requires a provably read-only FD")
def test_signing_rejects_nonzero_or_live_preflight(field: str, value: object, tmp_path: Path) -> None:
...
def test_key_domain_reuse_and_stale_preflight_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(OfflineSigningError, match="PREFLIGHT_NOT_ZERO_OR_FRESH"):
        require_fresh_zero_preflight_v1(canonical_json_bytes(signed), pin=pin, now=datetime(2026, 8, 9, 0, 1, tzinfo=timezone.utc))
...
def test_preflight_signing_recomputes_canonical_source_facts(tmp_path: Path) -> None:
    ...
    pin = _pin(private)
    descriptor = _key_fd(tmp_path, private)
    bad_facts = canonical_json_bytes({"pending_send_outcomes": 0, "active_orders": [], "positions": [{"symbol": "rb"}]})
    draft = _draft(pin)
    ...
    core = hashlib.sha256(canonical_json_bytes({key: value for key, value in draft.items() if key not in {"receipt_id", "receipt_core_sha256"}})).hexdigest()
    draft["receipt_core_sha256"] = core
    draft["receipt_id"] = "windows-fence-preflight-" + core
    try:
        with pytest.raises(OfflineSigningError, match="PREFLIGHT_NOT_ZERO_OR_FRESH"):
            sign_artifact_with_fd_v1(
                draft, private_key_fd=descriptor, pin=pin,
                execution_facts_raw=bad_facts, snapshot_raw=bad_facts,
            )
    finally:
        os.close(descriptor)
