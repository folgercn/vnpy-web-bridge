...
def test_catalog_path_swap_during_validation_fails_closed(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    values = sealed_evidence(tmp_path)
    ...
    replacement = derived.temporary / "replacement.duckdb"
    replacement.write_bytes(b"corrupt replacement")
    replacement.chmod(0o600)
    real_connect = duckdb.connect
    swapped = False

    def swap_after_private_catalog_open(database=":memory:", *args, **kwargs):
        nonlocal swapped
    ...
    monkeypatch.setattr(
        ...
        "connect",
        swap_after_private_catalog_open,
    )
    with pytest.raises(RegistryError, match="catalog changed during validation"):
        verify_rebuilt_catalog(
            evidence=values[0],
            derived=derived,
            public_key_path=values[1],
