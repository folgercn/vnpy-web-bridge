...
def _concurrent_command(
    state_path: str, receipt: dict, idempotency_key: str, queue
) -> None:
    service = PhaseCExecutionService(
        ExecutionSettings(
            Path(state_path), "execution-secret", "http://custody", "custody-secret"
        ),
    ...
    try:
        service.command(
            AuthorizationCommandDTO(
                ...
                expected_version=0,
                action="enable",
                authorization_artifact_id="artifact-1",
                custody_receipt_id="custody-install-1",
...
def signed_and_service(tmp_path: Path) -> tuple[dict, ArtifactCustodyService]:
    ...
    policies = {
        domain: CustodyPolicy(str(keyring_path), sha256_bytes(raw), purpose)
        for domain, (_kind, _schema, purpose) in {
            "map_acceptance": ("", "", "unused"),
            "c_fast_acceptance": ("", "", "unused"),
            "runtime_authorization": ("", "", "phase-c-runtime-authorization"),
        }.items()
    }
...
def test_custody_verifies_public_key_signature_request_binding_and_durable_install(
    tmp_path: Path,
) -> None:
    signed, service = signed_and_service(tmp_path)
    receipt = service.publish_install(
    ...
    with pytest.raises(WorkflowAdapterError):
        service.publish_install(
            SignedArtifactUploadDTO(
                idempotency_key="custody-key-0002",
                expected_custody_version=2,
                signing_request_id="wrong-request",
...
def test_execution_service_consumes_only_custody_receipt_and_keeps_runtime_disabled(
    tmp_path: Path,
) -> None:
    receipt = {
        "receipt_id": "custody-install-1",
    ...
    with TestClient(app) as client:
        response = client.post(
            ...
            headers={
                ...
                "X-Phase-C-Execution-Secret": "execution-secret",
            },
        )
...
def test_execution_same_expected_version_is_cross_process_atomic(
    tmp_path: Path,
) -> None:
    receipt = {
        "receipt_id": "custody-install-1",
    ...
    processes = [
        context.Process(
            target=_concurrent_command,
            args=(str(tmp_path / "state.json"), receipt, f"idem-key-000{i}", queue),
        )
        for i in (1, 2)
    ]
...
@pytest.mark.parametrize(
    ("change", "allowed_scope"),
    [
        ({"expires_at": "2000-01-01T00:00:00Z"}, {}),
        ({"scope": {"environment": "wrong"}}, {"environment": "offline"}),
    ],
)
def test_execution_rejects_expired_or_scope_mismatched_receipt(
    tmp_path: Path, change: dict, allowed_scope: dict
) -> None:
    receipt = {
        "receipt_id": "custody-install-1",
        "receipt_type": "install",
