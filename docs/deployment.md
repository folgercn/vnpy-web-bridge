...
C2b 已在代码中接线唯一 Commodity owner，在同一 `flock → Commodity cycle → RPC` 事务中从 actual custody 自动选择 reconciliation mode，冻结 owner/account/runtime/epoch/state identity。served-proof v2 由 owner-bound、不可 method-shadow 的真实 Linux RPC adapter 生成脱敏 canonical proof，保存 Windows cache/served/generation exact 字段、实际连接 endpoint/gateway 哈希、Linux connection/observed time 与 normalized facts hash；proof 先 create-only 持久化，activation head v2 再把它与 pair/marker 的 fresh facts 逐字节闭合。proof 明确记录 `windows_response_authenticated=false`：本阶段只证明本地 adapter 校验并接收响应，不声称 Windows 端密码学身份；该身份必须由后续 Windows trust/fence 阶段补足。v1 DTO/schema/raw 保持不变；v1 head 仅可审计，不能 bootstrap/C2c，也不能在 proof 缺失后回填。生产 fake RPC、配置错配、method shadow 或 bare DTO 在 artifact 写入前拒绝。它仍不证明卷外 high-water、目标镜像/config/container 身份，不释放 Windows fence，也不恢复 authority；代码合并不表示 Windows 扩展或 Linux runtime 已部署。
