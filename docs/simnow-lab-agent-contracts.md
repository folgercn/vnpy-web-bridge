...
本文件从 `86c7b16` 的根 AGENTS 迁移，保存既有执行、安全、预算与流程契约，不作为每个任务的全量必读材料。

- 根 `AGENTS.md` 确定按需入口；本次明确的 Issue/需求确定当前目标。阶段完成不解除冻结契约，但旧目标、验收进度与相对期限不自动成为新任务或授权。
- #462 的 M1–M3、#466 的 M4、#473 的 M5 预算与允许路径分别适用于对应阶段；M4/M5 明确扩展或修正的条款只在相应路径生效，未变更的执行、安全及冻结边界继续有效。不得把各阶段预算混加或自行豁免。
- M5 的自动 producer/roll 以第 0 节及 [Issue #473](https://github.com/folgercn/vnpy-web-bridge/issues/473) 为准；M4 的 reader/CD 看第 0.1 节及 [Issue #466](https://github.com/folgercn/vnpy-web-bridge/issues/466)；M1–M3 看第 2–7 节及 [Issue #462](https://github.com/folgercn/vnpy-web-bridge/issues/462)。
- 规程、当前任务或现场事实冲突且影响操作时，指出具体条款，只停止受影响操作；不能凭本文件扩大账户、主机、路径或 mutation 授权。

## 0. M5 契约与自动目标生产

- **Issue #473 的 M5 契约**：#462 的执行面与 #466 的 Dashboard、M2 runtime/CD、单个 LaunchAgent 继续冻结。
...
- #473 只在明确的自动 producer/roll 路径覆盖 M4/M1–M3 对应条款，不按章节位置推断其他授权。

M5 验收标记（不作为新任务目标或已完成证明）：
...
- 复用 `scripts/simnow_experimental_monthly_once.py`、`scripts/simnow_experimental_materialize_target.py`、`scripts/simnow_experimental_timely_daily_route.py` 和现有 Lab `run-once`，不重写算法或 target schema。
- 现有 one-shot 在 `09:05 / 13:35 / 21:05`（Asia/Shanghai）先 produce/确认 target 再 run-once。只在 target_id 变化时原子更新 `simnow_lab_evidence/experimental-target.json`；移除 target 自写入引起的 WatchPaths 递归唤醒，不能恢复第 0.1 节 M4 的自触发方式。
- producer 输入缺失、无效或非当前可用时，STOP before CURRENT/apply；保留最后合法 target 与已有持仓，不覆盖、不发送订单。不开设收市自动平仓路径。
- `source_month` 来自合法 producer 输入，不按墙钟猜测；同月 bundle create-only、字节稳定。同月 route 变化时 10 品种 quantity 逐项不变，只有新合法 source_month 可产生新 quantity vector。禁用测试 target、手工 quantity override 或直接编辑 target JSON。
- 每次只消费最新有效 DAILY PIT route content；route 不变不重写，变化只换 exact contract。固定实际输入/输出路径，不动态扫描整盘寻找最新文件；内容输出缺失时只允许复用既有 pure route kernel 的最小 content-only one-shot，不新增 signer/catalog/Custody/Research/ACL/sudo 层。
- exact-contract roll 必须旧合约先归零，取得 fresh positions/active orders 后再开新合约至原 monthly quantity；旧合约关闭失败、UNKNOWN 或 active order 未清零时不得开新。复用 SHFE/INE CLOSETODAY/CLOSEYESTERDAY，保持 quantity change 与 same-target NOOP，不新增 roll 状态机、ledger 或 successor。
- M5 production/config 最多 8 个文件、focused test 最多 3 个；target producer glue ≤ 300 行、executor roll diff ≤ 120 行，不机械压缩代码凑预算，超出立即 STOP。允许范围限定为 AGENTS、现有 one-shot 的薄组合、既有 plist/CD 必需清单、`executor_v1.py` 的最小 roll 修改和 focused tests，完整验收见 #473。
...
本节的 full CI 指无关全套检查，不取消第 12 节现有 required `CI Gate`。合并、部署与运行仍以本次有效授权为界。
...
只用于既有授权条件满足的 SIMNOW_LAB 开市 blocker；第 10 节 STOP 与适用阶段的路径/预算同时生效。M5 的限定 hotfix 依 #473 允许范围，不由此扩展到其他路径。
...
仅继续 #462 M1–M3 时使用以下开工检查；其他任务按当前 Issue 与适用阶段核对：
...
以下为迁移时的入口记录，不是本次连通性或服务状态证明；只在已授权操作需要时核对。
