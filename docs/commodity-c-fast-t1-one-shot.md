...
signed release
  ...
  -> create-only exact child invocation
...
exit 0 本身不能构成 P0 pass。runner 在 child 前先 create-only 保存 canonical
JSON child argv 到 `verified-bundle/release/child-invocation.json`，并在 child
前后重验其 exact bytes 和固定参数结构。terminal 的
`child_invocation_sha256` 必须等于该文件原始字节 SHA256，并同时绑定 signed
release、consume marker、四个产物的精确字节 SHA256、endpoint、QuestDB
build 和所有零写入/零交易 invariants。terminal 的 `started_at` 精确等于
consume marker 的 `consumed_at`。已有成功或 blocker terminal 只有在重新核对
consume 和 child invocation exact bytes、release/attempt/manifest/custody
bindings 及 state semantics 后才会被报告。
...
manifest、四个 schema bytes 和 exact child invocation create-only staged
到私有 attempt bundle；
输出进入单独可写 `artifacts/` 子目录，attempt parent 随后收紧为 `0500`，
防止整体替换 verified bundle。child 前后还会重核全部 staged bytes，包括
child invocation。子进程不再打开原始可变代码/manifest 路径。审计脚本会在
读取 staged manifest 后、读取 DSN/连接网络前核对 signed canonical manifest
SHA256。
