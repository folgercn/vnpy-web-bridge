...
输入清单必须是 `commodity_c_fast_l1_l5_audit_manifest_v1`，且正好包含十个唯一品种。`exact_contract` 可以使用研究侧 `SHFE.ag2609` 格式，也可以使用 vn.py `ag2609.SHFE` 格式；证据统一保留两种规范化表示。

```json
{
  "schema_version": "commodity_c_fast_l1_l5_audit_manifest_v1",
  "candidate_id": "C_FAST_CROSS_SECTION_NEUTRAL",
...
每个品种至少需要一个 execution window，否则 P0 保持 blocker。时间必须带明确 UTC offset，窗口必须完整落在命令行的 `[start, end)` 内。
...
1. JSON：完整不可变 evidence，结构由
   `docs/schemas/commodity-c-fast-l1-l5-audit-v1.schema.json` 定义；
