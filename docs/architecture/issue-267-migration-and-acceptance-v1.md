...
1-pre-C C2b 已在代码中接线唯一 `CommoditySimNowService` owner：在同一 `deployment flock → Commodity cycle → RPC` 事务里，从 C2a fd-pinned actual custody 自动选择唯一 C1a/C1b/C1c mode，冻结 owner/runtime/epoch/account allowlist/RPC endpoints/Commodity state identity，按确定性 intent 完成两拍 Windows capture，并以 create-only blob、marker 和每个 state head 唯一的 activation head 提交。初始 capture 不接受 cache replay；显式 fresh recheck 即使允许 cache replay，也必须把 Windows 返回的 `cache_replayed`、`served_at_utc`、`served_fact_generation` exact bytes 纳入 C2b create-only closure，生产路径还必须证明 DTO 只能由已校验该 served proof 的 transport 构造。当前代码只在 transport 瞬时校验这些字段，尚未持久化，因此这是 C2b runtime activation 前的 residual blocker，不能宣称 fresh-served 已形成可审计闭包。任一 generation、server、account、orders/trades/positions、runtime、epoch、state/anchor/inventory 或 owner identity 漂移都在 activation head 前 fail closed。C2b 只记录 `owner_reconciliation_activation_recorded=true`，不改 DeploymentDrain state，不释放 Windows fence，不恢复 worker/authority/deploy；`external_high_water_verified`、`target_runtime_verified`、`reconciliation_completed`、`windows_fence_released`、`authority_restore_allowed` 和所有 production/live/countable 字段仍为 false。C2b 合并不等于扩展已部署或运行激活。
