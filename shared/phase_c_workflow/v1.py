...
PHASE_C_DOMAINS = frozenset(
    {"map_acceptance", "c_fast_acceptance", "runtime_authorization"}
)
ARTIFACT_POLICY = {
    "map_acceptance": (
        "map-acceptance",
        "phase-c-map-acceptance-v1",
        "phase-c-map-acceptance",
    ),
    "c_fast_acceptance": (
        "c-fast-acceptance",
        "phase-c-c-fast-acceptance-v1",
        "phase-c-c-fast-acceptance",
    ),
    "runtime_authorization": (
        "runtime-authorization",
        "phase-c-runtime-authorization-v1",
        "phase-c-runtime-authorization",
    ),
}
...
_SENSITIVE_KEY = re.compile(
    r"(?:private|secret|credential|password|account|rpc|gateway|token|key|signature|order|position|trade)",
    re.IGNORECASE,
)
...
def validate_phase_c_artifact(
    ...
    try:
        ...
        raise PhaseCWorkflowError(
            "artifact envelope is invalid or authoritative"
        ) from exc
    ...
    if (
        ...
        envelope["schema_ref"],
    ) != (domain, artifact_type, schema_ref):
